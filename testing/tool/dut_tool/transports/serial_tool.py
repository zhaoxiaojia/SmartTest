"""
This module implements serial tool functionality for the ``dut_tool`` package.

It provides a ``serial_tool`` class which wraps low鈥憀evel ``pyserial`` primitives
...
class serial_tool:
    ...
    def __init__(self, serial_port='', baud='', log_file="kernel_log.txt", enable_log=True):
        ...
        self._rx_queue = Queue()  # Thread鈥憇afe receive buffer
            ...
            try:
                ...
                while time.time() - start_time < 10:
                    try:
                        ...
                        chunk = data.decode('utf-8', errors='ignore')
                        ip_info += chunk
                        # 濡傛灉鐪嬪埌 TX bytes 鎴栬€呮柊鎻愮ず绗︼紝鍙互鎻愬墠缁撴潫
