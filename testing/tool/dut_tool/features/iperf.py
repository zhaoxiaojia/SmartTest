...
class IperfFeature(FeatureBase):
    ...
    def _extract_udp_metrics(line: str) -> Optional[IperfMetrics]:
        sanitized = IperfFeature._sanitize_iperf_line(line)
        ...
        throughput = None
        if bandwidth_match:
            throughput = IperfFeature._convert_bandwidth_to_mbps(
    ...
    def _parse_iperf_log(self, server_lines: list[str]):
        def _analyse_lines(lines: list[str]) -> tuple[Optional[float], Optional[IperfMetrics], int, bool]:
            ...
            for raw_line in lines:
                line = IperfFeature._sanitize_iperf_line(raw_line)
                if not line:
                    continue
        ...
        if expected_intervals:
            ...
            while time.time() < wait_deadline:
                ...
                for text in snapshot:
                    sanitized = IperfFeature._sanitize_iperf_line(text)
                    if not sanitized:
                        continue
