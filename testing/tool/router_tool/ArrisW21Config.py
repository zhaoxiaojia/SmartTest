...
from .AsusRouter.AsusTelnetNvramControl import AsusBaseControl
...
class ArrisTelnetWlControl(AsusBaseControl):
    ...
    def __init__(
            ...
            *,
            ...
            prompt: bytes = b"> "  # 馃憟 Merlin 瀹屾暣 shell 鐨勫吀鍨嬫彁绀虹
    ) -> None:
        super().__init__(router_key, display=display, address=address)
        if address is not None:
            self._arris_router_ip = address
        else:
            # 濡傛灉娌′紶 address锛屽啀鍥為€€鍒扮埗绫荤殑閫昏緫
            self._arris_router_ip = router_key

        self.xpath = {
            "user": "admin",  # 濡傛灉 Telnet 闇€瑕佺敤鎴峰悕
            "passwd": "88888888"  # 鏇挎崲涓烘偍鐨勭湡瀹炲瘑鐮?
        }
        CORRECT_ROUTER_IP = "192.168.7.1"
        self.prompt = prompt
        self.telnet = None
        # self.telnet = TelnetSession(
        #     host=CORRECT_ROUTER_IP,  # 馃憟 鍏抽敭锛氫娇鐢ㄧ埗绫昏缃ソ鐨?self.address
        #     port=23,
        #     timeout=10
        # )
    ...
    def _login(self) -> None:
        ...
        try:
            ...
            # === 鍏抽敭淇锛氬湪 try 鍧楀唴瀹屾垚鎵€鏈夎缃?===
            # 1. 鍏堟爣璁颁负宸茬櫥褰曪紝閬垮厤鍚庣画鎿嶄綔瑙﹀彂閲嶈繛
            self._is_logged_in = True

            # 2. 鎵嬪姩鍒囨崲鍒?shell 妯″紡
            # 娉ㄦ剰锛氳繖閲屼笉鑳界敤 self.telnet_write锛屽洜涓哄畠浼氳Е鍙?_ensure_connection锛?
    ...
    def set_2g_channel(self, channel: Union[str, int]) -> None:
        ...
        if channel == "auto":
            self.telnet_write("nvram set wl0_channel_auto=1") # 0=鍏抽棴auto锛?=寮€鍚痑uto
        else:
            self.telnet_write(f"nvram set wl0_channel={channel}")
            self.telnet_write("nvram set wl0_channel_auto=0") # 0=鍏抽棴auto锛?=寮€鍚痑uto
