...
def _issue(issue_id: str, *, title: str = "") -> RedmineIssue:
    return RedmineIssue(id=issue_id, key=issue_id, title=title)


def test_redmine_issue_defaults_and_json_round_trip() -> None:
    issue = RedmineIssue(
        id="42",
        key="RM-42",
    ...
    round_tripped = RedmineIssue.from_dict(json.loads(json.dumps(serialized)))

    assert round_tripped == issue
    assert RedmineIssue().to_dict() == {
        "id": "",
        "key": "",
        "source_url": "",
        "title": "",
...
def test_redmine_issue_rejects_invalid_detail_state() -> None:
    with pytest.raises(ValueError, match="detail_state"):
        RedmineIssue(detail_state="ready")
...
def test_store_rejects_blank_issue_ids_atomically(blank_id: str) -> None:
    store = RedmineIssueStore([_issue("kept")])
...
def test_upsert_and_patch_preserve_existing_positions() -> None:
    store = RedmineIssueStore([_issue("1"), _issue("2")])
...
def test_unknown_ids_are_handled_deterministically() -> None:
    store = RedmineIssueStore([_issue("1")])
