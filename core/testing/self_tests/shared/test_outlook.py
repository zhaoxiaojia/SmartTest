...
from core.email.outlook import (
    OutlookAddressError,
    OutlookContentError,
    OutlookSendError,
    build_email,
...
from core.email.outlook.renderer import render_body
from core.email.outlook.sender import send_built_email
...
def test_sender_logs_safe_stages_and_rejects_partial_delivery(monkeypatch):
    ...
    records = []
    monkeypatch.setattr("core.email.outlook.sender.smart_log", lambda message, **kwargs: records.append((message, kwargs.get("extra", {}))))
    class FakeSmtp:
...
def test_send_email_logs_only_safe_message_counts(monkeypatch):
    ...
    monkeypatch.setattr("core.email.outlook.sender.send_built_email", lambda _built: None)
...
def test_daily_report_reuses_public_fixed_outlook_delivery_entrypoint():
    from core.tools.common.daily_report import service
