...
def test_project_catalog_import_does_not_load_retired_audit_runtime():
    result = subprocess.run(
        [
            ...
            (
                "import sys; "
                "import core.confluence.project_catalog; "
                "assert 'core.tools.common.project_weekly_audit.service' not in sys.modules"
            ),
        ],
        cwd=ROOT,
        capture_output=True,
...
def test_search_pages_paginates_and_normalizes():
    api = FakeConfluence()
    client = ConfluenceGateway(ConfluenceGatewayConfig("https://confluence.example"), "u", "secret", api=api)
...
def test_page_and_children_are_normalized():
    ...
    children = client.get_page_children("10")
...
def test_user_display_name_reuses_atlassian_user_lookup():
    ...
    client = ConfluenceGateway(
        ConfluenceGatewayConfig("https://confluence.example"), "u", "secret", api=UserApi(),
    )
...
def test_display_page_url_uses_one_targeted_title_request_with_catalog_expand():
    ...
    api = ExportViewApi()
    client = ConfluenceGateway(
        ConfluenceGatewayConfig("https://confluence.example"),
        "u", "secret", api=api,
    )
...
def test_page_by_url_prefers_static_export_view_with_one_targeted_title_request():
    ...
    api = ExportViewApi()
    client = ConfluenceGateway(
        ConfluenceGatewayConfig("https://confluence.example"), "u", "secret", api=api,
    )
...
def test_password_never_leaks_from_construction_failure(monkeypatch):
    ...
    monkeypatch.setattr("core.confluence.gateway.Confluence", Broken)
    with pytest.raises(RuntimeError) as captured:
        ConfluenceGateway(ConfluenceGatewayConfig("https://confluence.example"), "u", "top-secret")
...
def test_get_page_by_url_resolves_page_id_and_display_urls(url, expected_id):
    client = ConfluenceGateway(
        ConfluenceGatewayConfig("https://confluence.example"), "u", "secret",
        api=FakeConfluence(),
    )
