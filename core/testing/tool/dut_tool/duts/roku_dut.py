...
from core.testing.tool.dut_tool.duts.linux import linux
...
def decode_ignore(info: Annotated[bytes, "Byte string to decode"]):
    ...
    The implementation decodes the incoming byte array using UTF鈥?, replacing
...
class roku(linux):
    ...
    def _refresh_ip(self, ip: str) -> None:
        ...
        self.dut_ip = ip
        # --- 銆愪慨澶嶃€戞洿鏂扮幇鏈?roku_ctrl 瀵硅薄鐨?ip 鍜?telnet锛岃€屼笉鏄浛鎹㈠畠 ---
        self.roku.ip = ip
