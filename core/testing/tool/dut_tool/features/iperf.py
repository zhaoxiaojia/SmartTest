...
from core.testing.tool.dut_tool.command_batch import (
    CommandBatch,
...
def _extract_udp_metrics(line: str) -> Optional[IperfMetrics]:
    sanitized = _sanitize_iperf_line(line)
    ...
    throughput = None
    if bandwidth_match:
        throughput = _convert_bandwidth_to_mbps(
...
def _normalize_throughput_cells(self, entries: list[str]) -> list[str]:
    total_runs = max(1, int(self.repest_times))
...
def run_iperf(self, command, adb):
    ...
    use_adb = bool(adb)
    self._current_udp_mode = _is_udp_command(command)
    ...
    if "-s" in command:
        ...
        if use_adb:
            return _run_iperf_server_on_device(self, 
                command,
    ...
    if use_adb:
        return _run_iperf_client_on_device(self, 
            command,
...
def _parse_iperf_log(self, server_lines: list[str]):
    def _analyse_lines(lines: list[str]) -> tuple[Optional[float], Optional[IperfMetrics], int, bool]:
        ...
        for raw_line in lines:
            line = _sanitize_iperf_line(raw_line)
            if not line:
                continue
            if "[SUM]" not in line and self.pair != 1:
                metrics = _extract_udp_metrics(line)
                if metrics:
    ...
    if expected_intervals:
        ...
        while time.time() < wait_deadline:
            ...
            for text in snapshot:
                sanitized = _sanitize_iperf_line(text)
                if not sanitized:
                    continue
...
def get_rx_rate(self, router_info, type="TCP", corner_tool=None):
    if self.skip_rx:
        throughput_cells = _normalize_throughput_cells(self, ["0"])
    ...
    for _attempt in range(desired_runs + 2):
        ...
        try:
            if self.rvr_tool in ("iperf", "iperf3"):
                kill_iperf(self)
                terminal = run_iperf(self, self.tool_path + self.iperf_server_cmd, self.serialnumber)
                time.sleep(1)
                client_cmd = self.iperf_client_cmd.replace("{ip}", self.dut_ip)
                run_iperf(self, client_cmd, "")
                delay = _iperf_client_post_delay_seconds(self)
                if delay:
                    time.sleep(delay)
                rx_result = get_logcat(self)
                self.rvr_result = None
                if terminal:
                    terminal.terminate()
    ...
    throughput_cells = _normalize_throughput_cells(self, throughput_entries)
    ...
    for _attempt in range(desired_runs + 2):
        ...
        try:
            if self.rvr_tool in ("iperf", "iperf3"):
                kill_iperf(self)
                time.sleep(1)
                terminal = run_iperf(self, self.iperf_server_cmd, "")
                time.sleep(1)
                client_cmd = self.iperf_client_cmd.replace("{ip}", self.pc_ip)
                run_iperf(self, self.tool_path + client_cmd, self.serialnumber)
                delay = _iperf_client_post_delay_seconds(self)
                if delay:
                    time.sleep(delay)
                time.sleep(3)
                tx_result = get_logcat(self)
                self.rvr_result = None
                if terminal:
                    terminal.terminate()
            elif self.rvr_tool == "ixchariot":
