...
MANIFEST_SCRIPT = ROOT / "core/devtools/scripts/build_manifest.py"
...
def _manifest_module(tmp_path, version="2.3.4"):
    ...
    version_path = tmp_path / "core/release/version.json"
...
def test_three_package_consumers_use_the_shared_version(tmp_path):
    tool = runpy.run_path(str(ROOT / "client/scripts/build_tool_portable.py"))
    ...
    assert "core/release/version.json" in gradle
...
def test_release_workflow_validates_tag_before_environment_and_packaging():
    ...
    validation = "build_manifest.py --check-tag"
    assert validation in workflow
    assert workflow.index(validation) < workflow.index("init_venv.py")
