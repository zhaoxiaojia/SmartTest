...
def _check_logging_boundaries(root: Path = ROOT) -> list[str]:
    ...
    for product in ("client", "core", "web", "mobile"):
        for path in sorted((root / product).rglob("*.py")):
            ...
            if (
                "self_tests" in path.parts
                or "tests" in path.parts
                or "scripts" in path.parts
                or (root / "core" / "devtools") in path.parents
                or (root / "core" / "devtools" / "ci") in path.parents
                or (root / "core" / "logging") in path.parents
            ):
                continue
...
def _check_core(root: Path = ROOT) -> list[str]:
    ...
    for path in sorted((root / "core").rglob("*.py")):
        if (
            (root / "core" / "testing" / "self_tests") in path.parents
...
def _check_retired_support(root: Path = ROOT) -> list[str]:
    failures = []
    if (root / "support").exists():
        failures.append("support/: retired root owner must not exist")
    for product in PRODUCTS:
        for path in sorted((root / product).rglob("*.py")):
            tree = ast.parse(path.read_text(encoding="utf-8-sig"), filename=str(path))
            for node in ast.walk(tree):
                if isinstance(node, ast.Import) and any(alias.name.partition(".")[0] == "support" for alias in node.names):
                    failures.append(f"{path.relative_to(root)}: retired support import")
                elif isinstance(node, ast.ImportFrom) and node.level == 0 and node.module and node.module.partition(".")[0] == "support":
                    failures.append(f"{path.relative_to(root)}: retired support import")
    return failures
...
def main() -> int:
    ...
    failures.extend(_check_retired_support())
