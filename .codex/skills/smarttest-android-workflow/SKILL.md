...
description: Use when changing SmartTest mobile/android/, Kotlin APK cases, Android runner/device code, am-start parameters, shared step progress, Gradle APK builds, platform signing, priv-app installation, or desktop-to-APK integration.
---
...
- Give every case its own executor under `mobile/android/app/src/main/java/com/smarttest/mobile/runner/cases/`, grouped by domain when useful.
...
Use a stricter relevant task when required, platform-sign through repository assets/flow in `mobile/android/__init__.py`, copy the signed result under `dist/`, and remove stale ordinary debug artifacts when practical.
