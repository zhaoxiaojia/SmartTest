...
6. DUT refresh or DUT selection changes are a generic trigger only. The bridge must inspect the currently selected cases, collect their DUT-backed parameter `options_source` contracts, and refresh those sources through `testing/tool/dut_tool/parameter_adapter.py`; do not hard-code a case-specific DUT refresh path in QML or a page bridge.
