...
from jira.core.errors import JiraConfigurationError, JiraRequestError
from jira.core.models import JiraFieldMetadata, SearchPage
    ...
    def search_all(
        ...
        *,
        ...
        first_page = self.search_page(
            jql,
            start_at=0,
            max_results=page_size or self._config.page_size,
            fields=fields,
            expand=expand,
        )
        if first_page.total <= len(first_page.issues):
            return first_page.issues

        effective_page_size = first_page.max_results or self._config.page_size
        total_pages = ceil(first_page.total / effective_page_size)
...
def _trace_request(stage: str, **values: Any) -> None:
    ...
    print(f"[JIRA_HTTP] {stage} {details}".rstrip())
