...
import "../global"
    ...
    SystemTrayIcon {
        id:system_tray
        visible: true
        icon.source: "qrc:/example/res/image/taskbar_icon.png"
        tooltip: "SmartTest"
        menu: Menu {
            MenuItem {
                text: "退出"
    ...
    Flipable{
        ...
        front: Item{
            ...
            FluNavigationView{
                ...
                // Product pages declare SingleInstance so Settings/locale navigation keeps their local UI state.
                pageMode: FluNavigationViewType.Stack
                items: ItemsOriginal
    ...
    FluContentDialog{
        ...
        message:qsTr("SmartTest update ")+ newVerson +qsTr(" -- Current version ") +AppInfo.version+qsTr(" \n\nUpdated content: \n")+body
        buttonFlags: FluContentDialogType.NegativeButton | FluContentDialogType.PositiveButton
        negativeText: qsTr("Cancel")
        positiveText: qsTr("OK")
        onPositiveClicked:{
            if (AppInfo.updateDownloadUrl && AppInfo.updateDownloadUrl !== "") {
                Qt.openUrlExternally(AppInfo.updateDownloadUrl)
            }
        }
    }
    ...
    Callback{
        ...
        onError:
            (status,errorString)=>{
                if(!silent){
                    showInfo(qsTr("Update service is not configured"))
                }
