...
if __name__ == "__main__":
    ...
    subprocess.run([env.python(), os.path.join(scripts_dir, 'update_translations.py')], check=True)
    subprocess.run([env.python(), os.path.join(scripts_dir, 'update_resource.py')], check=True)

    repo_root = os.path.abspath(".")
    path = os.path.join(repo_root, "client", "app", "main.py")
    assets_dir = os.path.join(repo_root, "client", "packaging", "assets")
    args = [
        env.nuitka(),
        "--standalone",
        "--disable-console",
