...
### 11.4 QQ 式紧凑登录布局（验收修订）

用户验收否决了在旧表单上叠加账号下拉和“其他账号”按钮的布局。未登录视图改为以账号身份为视觉中心的紧凑结构：

1. 顶部显示当前选择账号的大尺寸圆形头像；没有缓存头像时显示姓名首字母。
2. 头像下方使用整宽账号选择框，主区域显示当前账号；展开列表的每项显示小头像、显示名、账号和保存凭据状态。
3. “添加其他账号”作为账号列表末尾动作，不再占用登录页独立按钮行。
4. 未保存密码或新增账号时显示整宽密码框；已有保存凭据并正在直接认证时隐藏密码框并显示进度。
5. “自动登录”和“保存密码”是两个独立选项：保存密码控制 Credential Manager；自动登录控制该账号是否允许在下次启动自动认证。启用自动登录必须同时具备已保存凭据，关闭自动登录不得删除凭据。
6. 主登录按钮使用整宽高强调样式；删除独立 LDAP Server 文本，服务信息只在错误或诊断中出现。
7. 背景、输入容器、弹出列表和主按钮使用 FluentUI 语义颜色并分别适配浅色、深色主题；参考 QQ 的视觉层级但不逐像素复制或引入新 UI 库。
8. 选择历史账号时，`AuthBridge` 必须立即从该账号缓存恢复 `avatarUrl`；只有明确移除账号时才删除其头像缓存。
9. 已登录账号资料面板保持现有信息结构，账号切换入口沿用同一账号模型，不复制一套列表状态。

运行时 UI 验收必须证明：顶部头像存在、选择不同历史账号后头像源同步变化、账号列表包含头像与“添加其他账号”、旧的独立按钮和 LDAP Server 文本已移除、密码与两个选项按账号状态正确显示。
