...
格式只审查 Confluence 结构是否存在，不审查字体、字号、颜色、粗体、HTML 标题级别或其他展示样式：

- 审查点要求某个子页面时，只要该子页面存在，格式即正确。
- 审查点要求页面内某个标题或字段时，只要该标题或字段存在，格式即正确；它可以由标题、普通段落、表格字段或宏标题承载。
- 字段后内容为空、`---` 或查询宏显示“未找到内容”不属于格式错误；该区域作为空内容继续参与本周更新比较。
- 只有要求的子页面、标题或字段完全不存在时，才输出 `格式有误：查询不到<标准名称>`。

`Summary of Experience and Typical Cases` 和 `Test Report Store` 以页面本身作为审查点，不强行追加页面中不存在的字段。页面存在即表示格式正确；页面正文可以为空。找不到对应子页面时输出 `格式有误：查询不到<页面标题>`；页面存在后再判断本周是否更新。

## 更新判定

1. 定位当前审查点要求的子页面、标题或字段，并把“结构是否存在”和“规范化内容”作为两个独立结果返回。
2. 结构不存在时，结果单元格写入 `格式有误：查询不到<标准名称>`；不再判断更新时间。
3. 结构存在时，即使规范化内容为空，也仅检查该目标区域在本周时间窗口内是否发生有效内容变化。
...
- 同一真实字段使用标题、普通段落、表格字段或宏标题展示时均能识别，字体和样式变化不影响格式判断。
- 页面、标题或字段存在但内容为空时不输出格式错误；空内容仍可与历史版本比较。

## 审查日志

- 每次运行记录审查周期、项目数、审查点数及各状态计数。
- 每个项目、每个审查点记录页面发现结果、页面标识、规则标识、定位器类型、命中元素类型、结构是否存在、规范化内容长度与 hash、当前及基线版本、历史比较结果、最终状态和原因。
- 页面读取或历史版本读取失败时记录异常类型、HTTP 状态及 page/version 标识。
- 日志使用现有 `smart_log` 结构化链路，不使用散落的 `print`，不记录正文全文、凭据、认证头或其他敏感数据。
