...
- 净增量（Round 2）：QML 增加 3 个最小 `objectName` 测试接口；离屏测试以可控 view state 验证确认/导出/定位的实际禁用状态，未新增 TS 语义条目。
- 测试：
...
- Round 2：`test_jira_audit_bridge.py`、`test_tool_page.py`、`test_owned_ui_translations.py` 组合测试退出码 0，`38 passed`；QML 资源已重新生成，`git diff --check` 退出码 0。
